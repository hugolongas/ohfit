...
<form enctype="multipart/form-data" id="colaborador_form" class="colaborador-form" action="{{ route('admin.colaboradores.update',$colaborador) }}"
    method="post">
    {{ method_field('PUT') }}
    {{ csrf_field() }}
    <div class="form-row">
        <div class="col-sm-6">
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="name" name="name" value="{{ old('name',$colaborador->name) }}"
                    placeholder="Colaborador...">
            </div>
            <div class="form-group">
                <label for="url">Enlace</label>
                <input type="text" class="form-control" id="url" name="url" value="{{ old('url',$colaborador->url) }}"
                    placeholder="https://....">
            </div>
        </div>
        <div class="col-sm-6">
            <div class="form-group">
                ...
                <input type="hidden" name="old_thumbnail" id="old_thumbnail" class="form-control"
                    value="{{ old('old_thumbnail',$colaborador->logo) }}" />
                <input name="logo" type="file" id="logo" onchange="readURL(this)" class="form-control" />
                <img id="img" src="{{asset('storage/colaborators').'/'.$colaborador->logo}}" alt="" style="width:100px " />
                @if($errors->has('logo'))
